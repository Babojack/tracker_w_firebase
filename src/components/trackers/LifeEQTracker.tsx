...
const LifeEQTracker: React.FC = () => {
  ...
  const [comparisonMonth, setComparisonMonth] = useState<string>(() =>
    getPrevMonth(new Date().toISOString().slice(0, 7))
  );
  ...
  useEffect(() => {
    const update = () => {
      ...
      setChartDimension({
        width: w < 640 ? 280 : w < 768 ? 320 : 400,
        height: w < 640 ? 280 : w < 768 ? 320 : 400
      });
    };
    update();
  ...
  // хелперы по категориям
  const updateCategory = (name: string, fn: (c: Category) => Category) =>
    setCategories((cs) => cs.map((c) => (c.name === name ? fn(c) : c)));
  ...
  const addTask = (catName: string) => {
    ...
    if (!txt) return;
    updateCategory(catName, (c) => ({
      ...c,
      tasks: [...c.tasks, { id: generateId(), text: txt, completed: false }]
    }));
    setNewTaskTexts((m) => ({ ...m, [catName]: "" }));
  };

  const toggleTask = (catName: string, id: string) =>
    updateCategory(catName, (c) => ({
      ...c,
      tasks: c.tasks.map((t) => t.id === id ? { ...t, completed: !t.completed } : t)
    }));

  const removeTask = (catName: string, id: string) =>
    updateCategory(catName, (c) => ({
      ...c,
      tasks: c.tasks.filter((t) => t.id !== id)
    }));

  const loadFor = (month: string): Category[] => {
    const raw = localStorage.getItem(`lifeEqCategories-${month}`);
    if (raw) {
      try {
        const parsed = JSON.parse(raw);
  ...
  const prevCats = useMemo(() => loadFor(previousMonth), [previousMonth]);
  const compCats = loadFor(comparisonMonth);
  const currCats = loadFor(currentMonth);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            {/* Новый ввод категории + кнопка под полем */}
            <div className="flex flex-col w-full sm:w-auto space-y-2">
              <input
                ...
                className="bg-gray-800 p-2 rounded text-sm sm:text-base min-w-[150px]"
              />
              <button
                onClick={addNewCategory}
                className="self-start bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-lg transition-colors"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {categories.map((cat) => {
              ...
              const d = cat.value - prev;
              const col = d > 0 ? "text-green-400" : d < 0 ? "text-red-400" : "text-gray-400";
              ...
              return (
                <div key={cat.name} className="bg-gray-700/50 p-4 rounded-lg space-y-3">
                  ...
                  <input
                    ...
                    max={10}
                    value={cat.value}
                    onChange={(e) => updateCategory(cat.name, (c) => ({ ...c, value: +e.target.value }))}
                    className="w-full h-2 rounded-lg appearance-none cursor-pointer"
                    style={{
                      background: `linear-gradient(to right, #3b82f6 ${cat.value * 10}%, #4b5563 ${cat.value * 10}%)`
                    }}
                  />
                  ...
                  <div className="space-y-2 pt-2 max-h-40 overflow-y-auto pr-2 custom-scroll">
                    {cat.tasks.map((t) => (
                      <div
                        key={t.id}
                        className="flex items-start justify-between text-xs sm:text-sm bg-gray-800/70 rounded px-2 py-1"
                      >
                  ...
                  {/* Поле + кнопка «New Task» под полем */}
                  <div className="flex flex-col space-y-2 pt-1">
                    <input
                      type="text"
                      value={newTaskTexts[cat.name] || ""}
                      onChange={(e) =>
                        setNewTaskTexts((m) => ({ ...m, [cat.name]: e.target.value }))
                      }
                      placeholder="New Task"
                      className="bg-gray-800 p-2 rounded text-xs sm:text-sm"
                    />
                    <button
                      onClick={() => addTask(cat.name)}
                      className="self-start bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-lg transition-colors"
                    >
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 bg-gray-700/50 rounded-lg p-4">
            ...
            <ResponsiveContainer width="100%" height={chartDimension.height}>
              <RadarChart data={loadFor(comparisonMonth)}>
                <PolarGrid strokeDasharray="3 3" />
                <PolarAngleAxis dataKey="name" tick={{ fill: "#9CA3AF", fontSize: 12, fontWeight: 500 }} />
                <PolarRadiusAxis domain={[0, 10]} tick={{ fill: "#9CA3AF" }} axisLine={{ strokeOpacity: 0.2 }} />
          ...
          <div className="flex-1 bg-gray-700/50 rounded-lg p-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="bg-gray-800 p-1 rounded text-sm">{currentMonth}</span>
            </div>
            <ResponsiveContainer width="100%" height={chartDimension.height}>
              <RadarChart data={loadFor(currentMonth)}>
                <PolarGrid strokeDasharray="3 3" />
                <PolarAngleAxis dataKey="name" tick={{ fill: "#9CA3AF", fontSize: 12, fontWeight: 500 }} />
                <PolarRadiusAxis domain={[0, 10]} tick={{ fill: "#9CA3AF" }} axisLine={{ strokeOpacity: 0.2 }} />
                <Radar name="Balance" dataKey="value" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.6} />
...
// хелпер для аналитики ниже компонента
function loadFor(month: string): Category[] {
  const raw = localStorage.getItem(`lifeEqCategories-${month}`);
  if (raw) {
    try {
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length) return parsed.map((c: any) => ({
        name: c.name,
        value: c.value,
